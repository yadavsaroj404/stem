...
export default function PauseModal({ onClose }: PauseModalProps) {
  ...
  return (
    <div className="z-10 fixed grid place-items-center inset-0  w-screen h-screen bg-black bg-opacity-50 p-4">
      <div className="w-full max-w-2xl rounded-4xl bg-[#230259] px-6 md:px-12 py-8 md:py-14 border-2 border-[#6300FF]/40">
        ...
        <p className="text-base font-normal w-full md:w-10/12 mx-auto text-center mt-2 mb-6">
          Your Progress is automatically saved. You can resume anytime within 7
          days.
        </p>

        <p className="text-lg md:text-xl text-center font-semibold mt-6 mb-4">
          We will send you a reminder email in:
        </p>
        <ul className="flex justify-center w-full md:w-2/3 mx-auto font-semibold gap-4 sm:gap-0">
        ...
        <div className="flex flex-col sm:flex-row justify-center sm:justify-between items-center mt-10 gap-6 sm:gap-0 sm:px-12">
          <button
            className="px-8 py-4 text-base border-2 border-gray-400 hover:border-white rounded-full font-semibold transition duration-200 flex items-center justify-center space-x-2 cursor-pointer w-full sm:w-auto"
            onClick={handleTakeABreak}
          >
            <span>Take a Break</span>
          </button>
          <button
            className="px-8 py-4 text-base border border-[#D400FF]/50 bg-gradient-to-r from-[#39008C] to-[#6400F8] rounded-full font-semibold transition duration-200 flex items-center justify-center space-x-2 cursor-pointer w-full sm:w-auto"
